...
impl<S: Read + Write> TlsStream<S> {
    ...
    fn handshake(&mut self) -> io::Result<()> {
        ...
        loop {
            ...
            // Прикладные данные, пришедшие в одном чтении с последней
            // пачкой рукопожатия, обязаны быть забраны здесь. Иначе они
            // расшифруются и пропадут: чтение начинается с пустого
            // буфера и уходит блокироваться на сокете, а отправитель
            // считает, что всё уже доставил.
            //
            // Прямому соединению это почти не грозит: пачка сервера и
            // ответ приходят разными сегментами. А вот через туннель
            // промежуточное звено их склеивает, и потеря становится
            // закономерной.

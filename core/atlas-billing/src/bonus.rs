...
mod tests {
    ...
    /// Рубли целым числом.
    ///
    /// Через минорные единицы, а не через `from_major`: тот отвечает
    /// `Option`, а разворачивать его здесь нечем — `expect` в этом крейте
    /// запрещён, и запрещён по делу. Паника в денежном коде недопустима и в
    /// тесте: тест — такой же код, и привычка «здесь-то можно» переезжает
    /// оттуда в рабочий путь.
    fn rub(major: u64) -> Money {
        Money::from_minor(major * 100, Currency::Rub)
    }

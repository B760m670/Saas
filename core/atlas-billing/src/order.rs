...
impl Plan {
    ...
    ///
    /// **Округление вниз, а не до ближайшего.** Разница невелика — годовой
    /// тариф даёт 46,72 %, и до ближайшего это 47, — но округление вверх
    /// означает витрину, обещающую больше, чем она даёт. Это претензия от
    /// покупателя, а не косметика, и одного процентного пункта достаточно,
    /// чтобы её предъявить.
    ///
    /// Занижение безопасно в обе стороны: покупатель получает чуть больше
    /// обещанного и никогда не меньше.
    #[must_use]
    pub fn discount_percent(&self, monthly_base: Money) -> Option<u32> {
        ...
        // saved / full в процентах, дробная часть отбрасывается.
        let percent = saved.checked_mul(100)?.checked_div(full)?;
...
mod tests {
    ...
    fn advertised_discounts_stay_truthful() {
        ...
        for (days, rubles, advertised) in SHOWCASE {
            ...
            assert_eq!(
                actual, advertised,
                "{days} дней: в документации −{advertised}%, считается −{actual}%"
            );

            // И главное: показанное число не должно превышать настоящее.
            // Проверяется без дробей — умножением накрест, иначе проверка
            // сама себе округляла бы то, что проверяет.
            let full = u128::from(base.minor()) * u128::from(days) / 30;
            let saved = full - u128::from(plan.price.minor());
            assert!(
                u128::from(actual) * full <= saved * 100,
                "{days} дней: обещаем −{actual}%, а даём меньше"
            );
            checked += 1;
        }

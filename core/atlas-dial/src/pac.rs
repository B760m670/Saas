    ...
    let mut build = || -> core::fmt::Result {
        ...
        out.push_str(
            ...
            "    var BLOCK = \"PROXY 0.0.0.0:0\";\n\
             ...
             \n\
             \x20   // Своё же устройство — мимо туннеля: иначе прокси\n\
             \x20   // попробует соединиться сам с собой. Ни одна из этих\n\
             \x20   // проверок не разрешает имя в адрес.\n\
             \x20   if (isPlainHostName(host)) return DIRECT;\n\
             \x20   if (host === \"localhost\" || shExpMatch(host, \"*.local\")) return DIRECT;\n\
             \n\
             \x20   // Диапазоны проверяются ТОЛЬКО у литеральных адресов.\n\
             \x20   //\n\
             \x20   // `isInNet` по спецификации разрешает имя в адрес. Вызвать\n\
             \x20   // её на доменном имени — значит заставить устройство\n\
             \x20   // сделать запрос DNS **мимо туннеля**, то есть показать\n\
             \x20   // провайдеру каждый посещённый домен. Измерено: пять\n\
             \x20   // разрешений на каждое имя.\n\
             \x20   //\n\
             \x20   // Имя, указывающее внутрь сети, при такой проверке пойдёт\n\
             \x20   // через туннель, а не напрямую. Это безопасная сторона\n\
             \x20   // ошибки: лишний виток лучше утечки.\n\
             \x20   if (/^[0-9]{1,3}(\\.[0-9]{1,3}){3}$/.test(host)) {\n\

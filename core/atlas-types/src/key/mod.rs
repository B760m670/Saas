...
pub struct RealityConfig {
    ...
    /// Открытый ключ ML-DSA-65 для дополнительной проверки сертификата.
    ///
    /// Параметр `pqv` в ссылке. Необязателен, и это часть замысла:
    /// клиент, не знающий про него, соединяется как раньше — просто без
    /// постквантовой проверки. Поэтому ключ с `pqv` можно раздавать, не
    /// разбираясь, что у кого установлено.
    pub post_quantum_verify: Option<String>,
}

...
impl Store {
    ...
    /// Занять право написать в поддержку. `true` — обращение идёт владельцу.
    ///
    /// `false` означает, что предыдущее обращение этого человека ещё без
    /// ответа. Второе письмо очередь не двигает, а владельцу вместо одного
    /// обращения достаётся шесть — и он читает их шесть раз вместо одного.
    ///
    /// `patience` — сколько ждём ответа, прежде чем вернуть право написать.
    /// Без этого срока молчание владельца запирало бы человека навсегда:
    /// проглядел одно обращение — и тот, у кого не работает оплаченный VPN,
    /// не может даже напомнить о себе. Молчание поддержки не повод лишать
    /// человека голоса.
    ///
    /// Право занимается одним запросом, а не «посмотреть и записать»: два
    /// сообщения подряд иначе проходят оба, и как раз от этого мы и
    /// защищаемся.
    pub fn open_support_request(
        &mut self,
        telegram_id: i64,
        now: i64,
        patience: i64,
        ...
        let changed = self.client.execute(
            "UPDATE users SET support_pending_at = to_timestamp($2::bigint)
              WHERE telegram_id = $1
                AND (support_pending_at IS NULL
                     OR support_pending_at <= to_timestamp($3::bigint))",
            &[&telegram_id, &now, &(now - patience)],
        )?;
        Ok(changed > 0)
    }

    /// Обращение закрыто: владелец ответил, человек снова может писать.
    ///
    /// Готовый ответ бота сюда не относится — он никого ни о чём не
    /// спрашивал и ничего не решил. Закрывает обращение только ответ,
    /// который владелец написал сам.
    pub fn close_support_request(&mut self, telegram_id: i64) -> Result<(), Error> {
        self.client.execute(
            "UPDATE users SET support_pending_at = NULL WHERE telegram_id = $1",
            &[&telegram_id],
        )?;
        Ok(())
    }
